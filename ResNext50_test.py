...
def load_backbone(model_name):
    ...
    if model_name == 'ResNeXt50':
        ...
        feature_dim = model.fc.in_features  # Get feature dimension from the final fully connected layer
    else:
...
def fine_tune_and_evaluate(model_name, dataloaders, test_loader, num_classes, num_epochs=50):
    ...
    for epoch in range(num_epochs):
        backbone.requires_grad = False  # Freeze backbone weights
        classifier.train()
        ...
        for inputs, labels in dataloaders['train']:
            ...
            # Forward pass
            with torch.no_grad():
                features = backbone(inputs)  # Extract features from the image using forward method
            features = features.view(features.size(0), -1)  # Flatten features for the classifier
...
# Example usage:
dataset_dir = '/content/drive/MyDrive/ML Project/RGB-M'
